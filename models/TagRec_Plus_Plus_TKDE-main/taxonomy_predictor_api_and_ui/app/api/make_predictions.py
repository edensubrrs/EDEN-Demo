...
model_path = configur['paths']['tagrec_model_path']

with open('config.ini', 'w') as config:
        configur.write(config)

sent_model = SentenceTransformer('bert-large-nli-stsb-mean-tokens')
#model_path_recommend = os.path.join(dir_path, 'modle')
model_path_recommend = model_path
...
model.load_state_dict(torch.load(model_path_recommend+'/model_weights.zip',map_location=torch.device('cuda')))
